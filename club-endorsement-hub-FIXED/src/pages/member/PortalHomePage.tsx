...
function ApplicantHome() {
  ...
  const mine = useQuery({
    ...
    queryFn: () =>
      apiRequest<Array<{ statusCode: string; applicantBallotLabel?: string | null }>>(
        "/api/applications/me",
      ),
    staleTime: 15_000,
    enabled: Boolean(user?.userAccountId),
  ...
  const meetingNotice =
    notifications.find((n) => n.typeCode === "MEETING_LINK") ??
    notifications.find((n) => n.typeCode === "INTERVIEW_MEETING");
  ...
  return (
    <PageFrame>
      ...
      {record?.id &&
      !["Waitlist", "ElectionReview", "TemporaryMember", "Committee", "CommitteeReview", "NotElected"].includes(
        status,
      ) ? (
