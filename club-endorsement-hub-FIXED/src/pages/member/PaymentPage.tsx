...
import { apiRequest, extractErrorMessage, fetchApplication } from "@/services/membership/api";
...
function ApplicantPaymentPage() {
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      if (methodCode === "CHEQUE" && !chequeNo.trim()) {
        throw new Error("Enter the cheque number.");
      }

      const noteParts = [reference.trim(), chequeBank.trim() ? `bank:${chequeBank.trim()}` : ""]
        .filter(Boolean)
        .join(" | ");
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {methodCode === "CHEQUE" ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <p className="sm:col-span-2 text-xs text-muted-foreground">
                    Cheque payments are stored as <strong>Pending</strong> in Payment_status until cleared.
                  </p>
                </div>
              ) : null}
              ...
              <Button
                type="button"
                className="w-full"
                disabled={submit.isPending || !applicationId || balanceForSelection(feeSelection) <= 0}
