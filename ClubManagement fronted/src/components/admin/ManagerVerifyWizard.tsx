...
export function ManagerVerifyWizard({
  ...
  row: ApplicationRow;
  onClose: () => void;
  onAuthorized?: (row: ApplicationRow) => void;
}) {
  ...
  const authorize = useMutation({
    ...
    onSuccess: () => {
      ...
      refresh();
      void queryClient.invalidateQueries({ queryKey: ["applications", "manager-queue"] });
      void queryClient.invalidateQueries({ queryKey: ["applications", "manager-history"] });
      onAuthorized?.(row);
      onClose();
    },
    onError: (err) => toast.error(extractErrorMessage(err)),
              ...
              {(payments.data ?? []).map((p, i) => (
                <li key={`${p.feeTypeCode ?? p.feeType}-${i}`} className="flex flex-wrap justify-between gap-2">
                  ...
                  <span className="text-muted-foreground">
                    {p.receiptNumber?.trim() || "No receipt"} · {money(Number(p.amount))}
                    {p.paymentDate ? ` · ${p.paymentDate.slice(0, 10)}` : ""}
                    {p.paymentStatus || p.status ? ` · ${p.paymentStatus || p.status}` : ""}
                  </span>
                </li>
              ))}
            </ul>
