...
export function readPortalMode(user: AuthUser | null = readUser()): PortalMode {
  ...
  if (!isStaff(user)) return isClubMember(user) ? "member" : "applicant";
  if (typeof window === "undefined") return "admin";
  const raw = window.localStorage.getItem(PORTAL_KEY);
  if (raw === "member" || raw === "applicant" || raw === "admin") return raw;
  return "admin";
}
...
export function homePathForUser(user: AuthUser | null): "/" | "/admin" {
  if (isStaff(user) && readPortalMode(user) === "admin") return "/admin";
  return "/";
}

/** True if any selected role needs a club membership number (Admin-only does not). */
export function roleRequiresMembershipNo(roleCodes: string | string[]) {
  ...
  return codes.some(
    (code) => code !== "ADMIN" && code !== "APPLICANT" && code !== "SUPER_ADMIN",
  );
}
